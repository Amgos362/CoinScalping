...
def buy(coin, vvr):
    ...
    if not avg_buy_price:  # 첫 번째 매수
        investment = krw * 0.25 * 0.9995  # 현금의 절반
    elif len(avg_buy_price) == 1:  # 두 번째 매수
        investment = krw * 0.3333 * 0.9995
    elif len(avg_buy_price) == 2: # 세 번째 매수
        investment = krw * 0.5 * 0.9995
    else: # 마지막 매수
