...
for i in range(len(coins)):
    ...
    ax1.axhline(y=120000, color='black', linestyle='--')  # indicator 10 위치에 선 추가
    ax1.axhline(y=880000, color='black', linestyle='--')  # indicator 90 위치에 선 추가
    ...
    for i in range(len(df.index) - 1):
        if df['indicator'][i] > 12 and df['indicator'][i+1] < 12 and red_marker == False:
            ax2.plot(df.index[i+1], df['close'][i+1], 'r^')
            red_marker = True
            blue_marker = False
        if df['indicator'][i] < 88 and df['indicator'][i+1] > 88 and blue_marker == False:
...
# # original 그래프 그리기
# ax3.plot(df.index, df['original'], color='tab:green')
# ax3.set_ylabel('Original', color='tab:green')  # y축 레이블 설정
# ax3.tick_params(axis='y', labelcolor='tab:green')  # y축 레이블 색상 설정
...
# import pyupbit
#
# access = "VxqVinDpK1iNlXCnOvSHCgXt24BSgUyiTRGkFv63"
# secret = "7Kros6MCQNxAjb78OYZNdXadAJEAxXeEJITx1aNG"
#
